...
tournaments_data = [
    {
        ...
        'status': 'Open',
        'max_slots': 50,
        'description': 'Biggest tournament of month with massive prize pool!',
...
if Tournament.objects.exists() and Player.objects.exists():
    tournament = Tournament.objects.first()
    players = Player.objects.all()[:3]
    
    for i, player in enumerate(players, 1):
        if not TournamentRegistration.objects.filter(tournament=tournament, slot_number=i).exists():
            TournamentRegistration.objects.create(
                ...
                payment_amount=tournament.entry_fee if tournament.entry_fee != 'Free' else '0'
            )
            print(f"Created registration for {player.name}")

print("\n✅ Django Admin Panel Setup Complete!")
...
print(f"   Registrations: {TournamentRegistration.objects.count()}")
